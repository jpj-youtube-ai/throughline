...
test("submitIdea can create a scratch idea (no voting until promoted)", async () => {
  ...
  try {
    ...
    const scratch = await listScratchIdeas(db, undefined, author);
...
test("scratch is author-scoped; promoteIdea opens it for voting and emits idea.graduated", async () => {
  ...
  try {
    ...
    assert.equal((await listScratchIdeas(db, undefined, bob)).length, 0);
    assert.equal((await listScratchIdeas(db, undefined, alice)).length, 1);
    ...
    assert.equal((await listScratchIdeas(db, undefined, alice)).length, 0);
